<template>
  <section class="column full-width q-pa-sm">
    <q-table title="Posts" :data="$store.getters.Categories.filter(m=>m._id !== cCategory.New)" :columns="columns" row-key="_id" virtual-scroll
...
import {CCategory, IPost} from "src/interfaces/IEssentials";
...
export default class Categories extends Vue {
  ...
  async deletePost(row: IPost) {
    await this.$store.commit("removeCategory", row)

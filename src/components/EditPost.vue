<template>
  <q-dialog v-model="showDialog" persistent>
    <q-card style="width: 600px; max-width: 90vw;">
      <q-form greedy @submit.prevent="editPost">
...
export default class EditPost extends Vue {
  ...
  created() {
    this.$root.$on('editPostModalOn', (row: IPost) => {
      ...
      this.post = row
      this.post.category = this.post.category.filter((each: string) => {
  ...
  editPost() {
    new Promise(async resolve => {
      resolve(await this.$store.commit('editPost', this.post))
    }).then(() => {
      this.showDialog = false
      showNotification(this.post.name + ' Edited!!')
    })
  }
